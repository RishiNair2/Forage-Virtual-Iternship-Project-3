...
SELECT customer_type, COUNT(loyalty_card_number) *100/(SELECT SUM(t1.total_customers)
													FROM (
													SELECT customer_type, COUNT(loyalty_card_number) total_customers
													FROM purchase_behavior
													GROUP BY customer_type
...
SELECT lifestage, COUNT(loyalty_card_number) *100/(SELECT SUM(t1.total_customers)
													FROM (
													SELECT lifestage, COUNT(loyalty_card_number) total_customers
													FROM purchase_behavior
													GROUP BY lifestage
													ORDER BY COUNT(loyalty_card_number) DESC
													) t1) percent_of_total
...
SELECT lifestage, customer_type, COUNT(loyalty_card_number) *100/(SELECT SUM(t1.total_customers)
													FROM (
													SELECT lifestage, customer_type, COUNT(loyalty_card_number) total_customers
													FROM purchase_behavior
